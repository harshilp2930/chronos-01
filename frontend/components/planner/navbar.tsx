...
const NAV_LINKS = [
  { href: "/dashboard/planner",              label: "Overview",    icon: LayoutDashboard, exact: true },
  { href: "/dashboard/planner/missions/new", label: "New Mission", icon: Plus, exact: true },
  { href: "/dashboard/planner/missions",     label: "History",     icon: History, excludes: ["/dashboard/planner/missions/new"] },
  { href: "/dashboard/planner/analytics",   label: "Analytics",   icon: BarChart3 },
  { href: "/dashboard/planner/reference",   label: "Reference",   icon: BookOpen },
];
...
export function PlannerNavbar() {
  ...
  const isActive = (href: string, exact?: boolean, excludes?: string[]) => {
    if (excludes?.some((e) => pathname.startsWith(e))) return false;
  ...
  return (
    ...
    >
      <div className="max-w-[1400px] mx-auto px-8 w-full flex items-center justify-between gap-6">
        ...
        {/* Logo */}
        <Link href="/dashboard/planner" className="flex items-center gap-2 shrink-0">
          ...
          <span className="text-sm hidden sm:inline" style={{ color: "rgba(240,244,255,0.5)" }}>
            {user?.role === "individual" ? "Individual Operator" : "Mission Planner"}
          </span>
        </Link>

        {/* Desktop nav */}
        <nav className="hidden md:flex items-center gap-1 flex-1 justify-center">
          {NAV_LINKS.map(({ href, label, icon: Icon, exact, excludes }) => {
            const active = isActive(href, exact, excludes);
            return (
              <Link
                key={href}
                href={href}
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {/* Moon icon (static dark mode indicator) */}
          <button
          ...
          <div className="relative">
            ...
            {dropdownOpen && (
              <>
                ...
                <div
                  className="absolute right-0 top-full mt-1.5 w-52 rounded-xl z-20 py-1 overflow-hidden"
                ...
                >
                  <div className="px-3 py-2.5" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                    ...
                    <span className="text-[10px] uppercase tracking-widest mt-1 block font-['JetBrains_Mono',monospace]" style={{ color: "#00E5FF" }}>
                      {user?.role === "individual" ? "Operator" : "Planner"}
                    </span>
                  </div>
                  <button
                    onClick={handleLogout}
                    className="w-full flex items-center gap-2 px-3 py-2 text-sm transition-colors hover:bg-[rgba(255,255,255,0.05)]"
